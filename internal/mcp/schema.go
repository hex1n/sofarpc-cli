...
func invokeInputSchema() map[string]any {
	return objectSchema(map[string]any{
		...
		"profile":              stringSchema("Select a Target Profile for this call. Outranks the session profile and the project's defaultProfile; empty falls back to them. A named-but-undefined profile is a hard error."),
...
func replayInputSchema() map[string]any {
	return objectSchema(map[string]any{
		...
		"planId":    stringSchema("Replay a specific plan from this session's captured history instead of the most recent one. Requires sessionId; planIds are returned by sofarpc_invoke's sessionPlanCapture diagnostics and the session resource."),
...
func initProjectInputSchema() map[string]any {
	return objectSchema(map[string]any{
		...
		"profile":             stringSchema("When set, write the target/wire fields into profiles[name], merged into the existing file (replacing only that profile key) rather than the base settings. allowedServices is never written in profile mode; force is needed only to overwrite an existing same-named profile."),
		"setDefault":          booleanSchema("When true with profile set, also record defaultProfile=profile in this file so calls that select no profile use it."),
