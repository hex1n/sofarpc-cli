...
func TestResolve_ProjectLocalDirectBeatsProjectDirect(t *testing.T) {
	report := Resolve(Input{}, Sources{
		Project:      Config{DirectURL: "bolt://project-host:12200"},
...
func TestResolve_InputDirectBeatsProjectDirect(t *testing.T) {
	report := Resolve(Input{
		DirectURL: "bolt://input-host:12200",
	}, Sources{
		Project: Config{DirectURL: "bolt://project-host:12200"},
...
func TestProjectSources_RejectsRegistryAddressField(t *testing.T) {
	root := t.TempDir()
	writeProjectTargetConfig(t, root, "config.json", `{
  "registryAddress": "zookeeper://zk:2181"
	...
	sources := ProjectSources(root, Config{})
	if len(sources.ConfigErrors) != 1 {
		t.Fatalf("registryAddress is no longer a config field and must be rejected: %+v", sources.ConfigErrors)
	}
